...
class RiskManager:
    def __init__(self, config: Config, storage: Storage) -> None:
        ...
        if config.starting_cash_usdh > 0:
            ...
            perp_ledger = self.storage.get_perp_ledger_state()
            if float(perp_ledger["cash_balance"]) == 0 and float(perp_ledger["equity"]) == 0:
                self.storage.update_perp_ledger_state(
                    cash_balance=config.starting_cash_usdh,
                    equity=config.starting_cash_usdh,
                    peak_equity=config.starting_cash_usdh,
                )
    ...
    def perp_snapshot(self) -> RiskSnapshot:
        ledger = self.storage.get_perp_ledger_state()
        current_equity = float(ledger["equity"])
        peak_equity = float(ledger["peak_equity"])
        drawdown = 0.0
        if peak_equity > 0:
            drawdown = max((peak_equity - current_equity) / peak_equity, 0.0)
        return RiskSnapshot(
            total_reserved_exposure=self.storage.total_reserved_perp_exposure(),
            peak_equity=peak_equity,
            current_equity=current_equity,
            drawdown=drawdown,
            open_markets=self.storage.open_perp_market_ids(),
        )
    ...
    def update_perp_balance(self, new_balance_usdh: float) -> None:
        ledger = self.storage.get_perp_ledger_state()
        peak_equity = max(float(ledger["peak_equity"]), new_balance_usdh)
        self.storage.update_perp_ledger_state(
            cash_balance=new_balance_usdh,
